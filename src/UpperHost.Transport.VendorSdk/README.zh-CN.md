...
`UpperHost.Transport.VendorSdk` 是**厂商 SDK 扩展的统一参考模板**。它只用于厂商 SDK 提供“有序字节会话”、但又不适合直接归类为 Serial/TCP/USB 的情况。
...
2. Native Callback 转入 UpperHost 时不能阻塞厂商回调线程。
