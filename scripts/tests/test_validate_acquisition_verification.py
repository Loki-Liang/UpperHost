...
def valid_profile():
    return {
        ...
        "mode": "VirtualTimeDeterministic",
        "sourceCount": 1,
        "channelsPerSource": 2,
        "numericType": "float32",
        "bytesPerSample": 4,
        "sampleRateHz": 1000,
        "blockSizeSamples": 100,
        "durationSeconds": 10,
        "expectedRawBytesPerSecond": 8000,
        "routerBranches": [
            {"id": "required", "delivery": "Required", "capacityBlocks": 8, "overflow": "Wait"},
            {"id": "ui", "delivery": "Optional", "capacityBlocks": 2, "overflow": "DropOldest"},
        ],
...
class AcquisitionVerificationValidatorTests(unittest.TestCase):
    ...
    def test_required_branch_cannot_be_lossy(self):
        profile = valid_profile()
        profile["routerBranches"][0]["overflow"] = "DropOldest"
        with self.assertRaises(MODULE.ValidationError):
            MODULE.validate_profile(profile, "test.json")

    def test_optional_branch_cannot_backpressure_required_work(self):
        profile = valid_profile()
        profile["routerBranches"][1]["overflow"] = "Wait"
        with self.assertRaises(MODULE.ValidationError):
            MODULE.validate_profile(profile, "test.json")
